  ...
  broadcaster_(this)
{
  ...
  declare_parameter("use_absolute_odom_pose_prediction", false);
  declare_parameter("absolute_odom_correction_gain", 0.1);
  declare_parameter("absolute_odom_max_time_delta_sec", 0.05);
  ...
  declare_parameter("publish_alignment_debug_output", false);
  ...
  declare_parameter("enable_correction_limit", false);
  declare_parameter("max_correction_translation_m", 1.0);
  declare_parameter("max_correction_yaw_deg", 10.0);
  declare_parameter("max_correction_rotation_deg", 180.0);
...
CallbackReturn PCLLocalization::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  alignment_seed_cloud_pub_->on_activate();
  alignment_proposed_cloud_pub_->on_activate();
  alignment_seed_pose_pub_->on_activate();
  alignment_proposed_pose_pub_->on_activate();
...
CallbackReturn PCLLocalization::on_deactivate(const rclcpp_lifecycle::State &)
{
  ...
  alignment_seed_cloud_pub_->on_deactivate();
  alignment_proposed_cloud_pub_->on_deactivate();
  alignment_seed_pose_pub_->on_deactivate();
  alignment_proposed_pose_pub_->on_deactivate();
...
void PCLLocalization::releaseRuntimeResources(bool leak_target_clouds_for_shutdown)
{
  ...
  alignment_seed_cloud_pub_.reset();
  alignment_proposed_cloud_pub_.reset();
  alignment_seed_pose_pub_.reset();
  alignment_proposed_pose_pub_.reset();
  path_pub_.reset();
  status_pub_.reset();
  ...
  {
    std::lock_guard<std::mutex> lock(absolute_odom_mutex_);
    absolute_odom_history_.clear();
    map_to_odom_correction_ = Eigen::Matrix4f::Identity();
    have_map_to_odom_correction_ = false;
  }
}
...
void PCLLocalization::initializeParameters()
{
  ...
  get_parameter("use_absolute_odom_pose_prediction", use_absolute_odom_pose_prediction_);
  get_parameter("absolute_odom_correction_gain", absolute_odom_correction_gain_);
  get_parameter("absolute_odom_max_time_delta_sec", absolute_odom_max_time_delta_sec_);
  if (!std::isfinite(absolute_odom_correction_gain_)) {
    RCLCPP_WARN(
      get_logger(), "absolute_odom_correction_gain must be finite; using 0.1");
    absolute_odom_correction_gain_ = 0.1;
  } else {
    absolute_odom_correction_gain_ = std::clamp(absolute_odom_correction_gain_, 0.0, 1.0);
  }
  if (!std::isfinite(absolute_odom_max_time_delta_sec_) || absolute_odom_max_time_delta_sec_ < 0.0) {
    RCLCPP_WARN(
      get_logger(), "absolute_odom_max_time_delta_sec must be finite and non-negative; using 0.05");
    absolute_odom_max_time_delta_sec_ = 0.05;
  }
  if (use_absolute_odom_pose_prediction_ && !use_odom_) {
    RCLCPP_WARN(
      get_logger(), "use_absolute_odom_pose_prediction requires use_odom; disabling absolute mode");
    use_absolute_odom_pose_prediction_ = false;
  }
  ...
  get_parameter("publish_alignment_debug_output", publish_alignment_debug_output_);
  ...
  get_parameter("enable_correction_limit", measurement_gate_config_.enable_correction_limit);
  get_parameter(
    "max_correction_translation_m",
    measurement_gate_config_.max_correction_translation_m);
  get_parameter("max_correction_yaw_deg", measurement_gate_config_.max_correction_yaw_deg);
  get_parameter(
    "max_correction_rotation_deg",
    measurement_gate_config_.max_correction_rotation_deg);
  ...
  RCLCPP_INFO(
    get_logger(), "use_absolute_odom_pose_prediction: %d",
    use_absolute_odom_pose_prediction_);
  RCLCPP_INFO(
    get_logger(), "absolute_odom_correction_gain: %lf", absolute_odom_correction_gain_);
  RCLCPP_INFO(
    get_logger(), "absolute_odom_max_time_delta_sec: %lf",
    absolute_odom_max_time_delta_sec_);
  ...
  RCLCPP_INFO(
    get_logger(), "enable_correction_limit: %d", measurement_gate_config_.enable_correction_limit);
  RCLCPP_INFO(
    get_logger(), "max_correction_translation_m: %lf",
    measurement_gate_config_.max_correction_translation_m);
  RCLCPP_INFO(
    get_logger(), "max_correction_yaw_deg: %lf",
    measurement_gate_config_.max_correction_yaw_deg);
  RCLCPP_INFO(
    get_logger(), "max_correction_rotation_deg: %lf",
    measurement_gate_config_.max_correction_rotation_deg);
  RCLCPP_INFO(
  ...
  RCLCPP_INFO(
    get_logger(), "publish_alignment_debug_output: %d",
    publish_alignment_debug_output_);
...
void PCLLocalization::initializePubSub()
{
  ...
  alignment_seed_cloud_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(
    "debug/alignment_seed_cloud", rclcpp::SensorDataQoS().keep_last(1));
  alignment_proposed_cloud_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(
    "debug/alignment_proposed_cloud", rclcpp::SensorDataQoS().keep_last(1));
  alignment_seed_pose_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>(
    "debug/alignment_seed_pose", rclcpp::SensorDataQoS().keep_last(1));
  alignment_proposed_pose_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>(
    "debug/alignment_proposed_pose", rclcpp::SensorDataQoS().keep_last(1));
...
void PCLLocalization::initialPoseReceived(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
  ...
  msg->header.stamp = normalizeInitialPoseStamp(msg->header.stamp);
  ...
  last_odom_received_time_ = last_initial_pose_stamp_sec_;
  initial_pose_generation_.fetch_add(1, std::memory_order_release);
  corrent_pose_with_cov_stamped_ptr_ = msg;
  if (
    use_absolute_odom_pose_prediction_ &&
    !resetMapToOdomCorrection(currentPoseMatrix(), last_initial_pose_stamp_sec_))
  {
    RCLCPP_WARN(
      get_logger(),
      "initial pose accepted, but no synchronized absolute odometry pose is available yet");
  }
  {
    std::lock_guard<std::mutex> lock(imu_preintegration_mutex_);
    imu_preintegration_fallback_mode_ = false;
...
builtin_interfaces::msg::Time PCLLocalization::normalizeInitialPoseStamp(
  const builtin_interfaces::msg::Time & requested_stamp)
{
  ...
  try {
    const auto latest_odom_to_base = tfbuffer_.lookupTransform(
      odom_frame_id_, base_frame_id_, tf2::TimePointZero);
    const auto & sensor_stamp = latest_odom_to_base.header.stamp;
    RCLCPP_INFO(
      get_logger(),
      "normalized initial pose stamp to latest %s -> %s sensor time %d.%09u",
      odom_frame_id_.c_str(),
      base_frame_id_.c_str(),
      sensor_stamp.sec,
      sensor_stamp.nanosec);
    return sensor_stamp;
  } catch (tf2::TransformException & ex) {
    RCLCPP_WARN(
      get_logger(),
      "could not normalize initial pose to the odometry clock yet: %s; "
      "the first sensor-stamped registration update will establish map -> odom",
      ex.what());
...
void PCLLocalization::odomReceived(const nav_msgs::msg::Odometry::ConstSharedPtr msg)
{
  ...
  if (use_absolute_odom_pose_prediction_) {
    Eigen::Matrix4f odom_to_base = Eigen::Matrix4f::Identity();
    if (!lidar_localization::poseMatrixFromPose(msg->pose.pose, &odom_to_base)) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 5000,
        "absolute odometry pose is non-finite or has an invalid quaternion; dropping it");
      return;
    }
    cacheAbsoluteOdomPose(current_odom_received_time, odom_to_base);
    ...
    if (!predictMapPoseFromAbsoluteOdom(current_odom_received_time, &map_to_base)) {
      if (!resetMapToOdomCorrection(currentPoseMatrix(), current_odom_received_time)) {
        return;
      }
      map_to_base = currentPoseMatrix();
    }
    setCurrentPoseFromMatrix(map_to_base, msg->header.stamp);
    return;
  }
  ...
  corrent_pose_with_cov_stamped_ptr_->pose.pose =
    lidar_localization::integrateBodyFrameTwist(
    previous_pose, msg->twist.twist, dt_odom);
...
void PCLLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  publishAlignmentDebugOutput(msg->header.stamp, tmp_ptr, pipeline_result.selected_attempt);
  ...
  double scan_stamp_sec)
{
  ...
  if (use_absolute_odom_pose_prediction_) {
    Eigen::Matrix4f absolute_odom_seed = Eigen::Matrix4f::Identity();
    if (predictMapPoseFromAbsoluteOdom(scan_stamp_sec, &absolute_odom_seed)) {
      selected_seed.init_guess = absolute_odom_seed;
    } else {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 5000,
        "no synchronized absolute odometry pose for scan seed; using current map pose");
    }
    selected_seed.source = lidar_localization::RegistrationSeedSource::kCurrentPose;
  ...
  double scan_stamp_sec)
{
  ...
  if (attempt.has_converged) {
    ...
    attempt.correction_rotation_deg = correction_metrics.rotation_deg;
  }
  ...
  const lidar_localization::AlignmentAttempt & attempt)
{
  const auto gate_input = lidar_localization::makeMeasurementGateInput(
    ...
    consecutive_rejected_updates_,
    attempt.correction_rotation_deg);
  auto gate =
  ...
  if (gate.status_level == lidar_localization::kMeasurementGateWarn) {
    RCLCPP_WARN(
      get_logger(),
      "measurement gate: %s (fitness=%.6f correction=%.3fm yaw=%.3fdeg rotation=%.3fdeg)",
      gate.status_message.c_str(),
      attempt.fitness_score,
      attempt.correction_translation_m,
      attempt.correction_yaw_deg,
      attempt.correction_rotation_deg);
...
void PCLLocalization::publishAlignmentDebugOutput(
  const builtin_interfaces::msg::Time & stamp,
  const pcl::PointCloud<pcl::PointXYZI>::Ptr & source_cloud,
  const lidar_localization::AlignmentAttempt & selected_attempt)
{
  if (!publish_alignment_debug_output_ || !source_cloud || source_cloud->empty() ||
    !selected_attempt.target_ready)
  {
    return;
  }

  auto publish_pose = [this, &stamp](
      const Eigen::Matrix4f & matrix,
      const rclcpp_lifecycle::LifecyclePublisher<geometry_msgs::msg::PoseStamped>::SharedPtr &
      publisher) {
      geometry_msgs::msg::PoseStamped message;
      message.header.stamp = stamp;
      message.header.frame_id = global_frame_id_;
      message.pose.position.x = matrix(0, 3);
      message.pose.position.y = matrix(1, 3);
      message.pose.position.z = matrix(2, 3);
      const Eigen::Quaternionf quaternion(matrix.block<3, 3>(0, 0));
  ...
  auto publish_cloud = [this, &stamp, &source_cloud](
      const Eigen::Matrix4f & matrix,
      const rclcpp_lifecycle::LifecyclePublisher<sensor_msgs::msg::PointCloud2>::SharedPtr &
      publisher) {
      pcl::PointCloud<pcl::PointXYZI> transformed;
      pcl::transformPointCloud(*source_cloud, transformed, matrix);
  ...
  publish_pose(selected_attempt.init_guess, alignment_seed_pose_pub_);
  publish_cloud(selected_attempt.init_guess, alignment_seed_cloud_pub_);
  if (selected_attempt.has_converged) {
    publish_pose(selected_attempt.final_transformation, alignment_proposed_pose_pub_);
    publish_cloud(selected_attempt.final_transformation, alignment_proposed_cloud_pub_);
  }
}
...
void PCLLocalization::cacheAbsoluteOdomPose(
  double stamp_sec,
  const Eigen::Matrix4f & odom_to_base)
{
  if (!std::isfinite(stamp_sec) || !odom_to_base.allFinite()) {
    return;
  }
  std::lock_guard<std::mutex> lock(absolute_odom_mutex_);
  if (
    !absolute_odom_history_.empty() &&
    stamp_sec < absolute_odom_history_.back().stamp_sec)
  {
    absolute_odom_history_.clear();
  }
  absolute_odom_history_.push_back({stamp_sec, odom_to_base});
  while (absolute_odom_history_.size() > absolute_odom_history_size_) {
    absolute_odom_history_.pop_front();
  }
}

bool PCLLocalization::lookupAbsoluteOdomPose(
  double stamp_sec,
  Eigen::Matrix4f * odom_to_base)
{
  std::lock_guard<std::mutex> lock(absolute_odom_mutex_);
  return lidar_localization::nearestTimedPose(
    absolute_odom_history_, stamp_sec, absolute_odom_max_time_delta_sec_, odom_to_base);
}

bool PCLLocalization::resetMapToOdomCorrection(
  const Eigen::Matrix4f & map_to_base,
  double stamp_sec)
{
  Eigen::Matrix4f odom_to_base = Eigen::Matrix4f::Identity();
  if (!lookupAbsoluteOdomPose(stamp_sec, &odom_to_base)) {
    return false;
  }
  const Eigen::Matrix4f map_to_odom =
    lidar_localization::mapToOdomFromPoses(map_to_base, odom_to_base);
  if (!map_to_odom.allFinite()) {
    return false;
  }
  std::lock_guard<std::mutex> lock(absolute_odom_mutex_);
  map_to_odom_correction_ = map_to_odom;
  have_map_to_odom_correction_ = true;
  return true;
}

bool PCLLocalization::predictMapPoseFromAbsoluteOdom(
  double stamp_sec,
  Eigen::Matrix4f * map_to_base)
{
  ...
  std::lock_guard<std::mutex> lock(absolute_odom_mutex_);
  if (!have_map_to_odom_correction_ || map_to_base == nullptr) {
    return false;
  }
  *map_to_base = lidar_localization::predictMapPoseFromAbsoluteOdom(
    map_to_odom_correction_, odom_to_base);
  return map_to_base->allFinite();
}

bool PCLLocalization::filterAcceptedMapPoseWithAbsoluteOdom(
  const Eigen::Matrix4f & measured_map_to_base,
  double stamp_sec,
  Eigen::Matrix4f * filtered_map_to_base)
{
  Eigen::Matrix4f odom_to_base = Eigen::Matrix4f::Identity();
  if (
    filtered_map_to_base == nullptr || !measured_map_to_base.allFinite() ||
    !lookupAbsoluteOdomPose(stamp_sec, &odom_to_base))
  {
    return false;
  }

  const Eigen::Matrix4f measured_map_to_odom =
    lidar_localization::mapToOdomFromPoses(measured_map_to_base, odom_to_base);
  if (!measured_map_to_odom.allFinite()) {
    return false;
  }

  std::lock_guard<std::mutex> lock(absolute_odom_mutex_);
  if (!have_map_to_odom_correction_) {
    map_to_odom_correction_ = measured_map_to_odom;
    have_map_to_odom_correction_ = true;
  } else {
    map_to_odom_correction_ = lidar_localization::blendRigidTransforms(
      map_to_odom_correction_, measured_map_to_odom, absolute_odom_correction_gain_);
  }
  *filtered_map_to_base = lidar_localization::predictMapPoseFromAbsoluteOdom(
    map_to_odom_correction_, odom_to_base);
  return filtered_map_to_base->allFinite();
}
...
void PCLLocalization::applyAcceptedPoseBackendResult(
  ...
  double fitness_score)
{
  Eigen::Matrix4f pose_to_apply = result.pose_matrix;
  if (
    use_absolute_odom_pose_prediction_ && result.update_current_pose &&
    !filterAcceptedMapPoseWithAbsoluteOdom(result.pose_matrix, stamp_sec, &pose_to_apply))
  {
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 5000,
      "accepted registration has no synchronized absolute odometry pose; holding map correction");
    return;
  }
  if (result.update_current_pose) {
    setCurrentPoseFromMatrix(pose_to_apply, stamp);
  }
  if (result.fill_pose_covariance) {
    fillPoseCovariance(fitness_score);
  }
  if (result.update_prediction_state) {
    updatePredictionState(pose_to_apply, stamp_sec);
  }
}
  ...
  const geometry_msgs::msg::TransformStamped & map_to_base_link_stamped)
{
  ...
  auto map_to_odom_transform = lidar_localization::composeMapToOdomTransform(
    stamp,
    global_frame_id_,
    odom_frame_id_,
  ...
  {
    std::lock_guard<std::mutex> lock(map_odom_transform_mutex_);
    last_map_to_odom_transform_ = map_to_odom_transform;
    have_last_map_to_odom_transform_ = true;
  }
...
void PCLLocalization::timerPublishPose()
{
  ...
  if (enable_map_odom_tf_) {
    try {
      publish_stamp = tfbuffer_.lookupTransform(
        odom_frame_id_, base_frame_id_, tf2::TimePointZero).header.stamp;
    } catch (tf2::TransformException & ex) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 5000,
        "timer publish waiting for latest %s -> %s sensor-stamped transform: %s",
  ...
  if (enable_map_odom_tf_) {
    geometry_msgs::msg::TransformStamped held_map_to_odom;
    {
      std::lock_guard<std::mutex> lock(map_odom_transform_mutex_);
      if (!have_last_map_to_odom_transform_) {
        return;
      }
      held_map_to_odom = last_map_to_odom_transform_;
    }
    held_map_to_odom.header.stamp = publish_stamp;
    broadcaster_.sendTransform(held_map_to_odom);
  } else {
