...
def _write_map_yaml(path: Path, image_name: str) -> None:
    path.write_text(
        yaml.safe_dump(
            {
                "image": image_name,
...
def test_ros_ascii_pgm_fallback_loader():
    with tempfile.TemporaryDirectory() as tmp:
        directory = Path(tmp)
        pgm_path = directory / "map.pgm"
        pgm_path.write_text(
            "P2\n# ROS map_server ASCII PGM\n3 2\n255\n254 205 0\n0 254 254\n",
            encoding="ascii",
        )
        yaml_path = directory / "map.yaml"
        _write_map_yaml(yaml_path, pgm_path.name)

        occupancy = bbs.load_occupancy_map(yaml_path)

        assert occupancy.occupied.tolist() == [
            [True, False, False],
            [False, False, True],
        ]


def test_binary_pgm_fallback_loader():
    with tempfile.TemporaryDirectory() as tmp:
        directory = Path(tmp)
        pgm_path = directory / "map.pgm"
        ...
        pixels = bbs._load_netpbm_grayscale(pgm_path)

        assert pixels.tolist() == [[0.0, 254.0], [205.0, 0.0]]
...
def main():
    ...
    test_ros_ascii_pgm_fallback_loader()
    test_binary_pgm_fallback_loader()
