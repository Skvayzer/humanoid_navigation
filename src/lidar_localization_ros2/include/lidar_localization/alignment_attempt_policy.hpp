...
inline double poseDeltaRotationAbsDeg(const Eigen::Matrix4f & delta_matrix)
{
  constexpr double kRadiansToDegrees = 180.0 / 3.14159265358979323846;
  const Eigen::AngleAxisf delta_rotation(delta_matrix.block<3, 3>(0, 0));
  return std::abs(static_cast<double>(delta_rotation.angle())) * kRadiansToDegrees;
}
  ...
  const Eigen::Matrix4f & final_transformation)
{
  ...
  return {
    ...
    poseDeltaRotationAbsDeg(correction_matrix)};
}
