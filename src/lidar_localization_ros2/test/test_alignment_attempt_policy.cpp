...
void test_pose_delta_helpers()
{
  ...
  assert(std::abs(ll::poseDeltaRotationAbsDeg(delta) - 90.0) < 1.0e-4);
...
void test_correction_metrics_use_init_to_final_delta()
{
  ...
  assert(std::abs(metrics.rotation_deg - 20.0) < 1.0e-4);
}

void test_rotation_metric_detects_tilt_hidden_from_yaw_metric()
{
  Eigen::Matrix4f tilted_pose = Eigen::Matrix4f::Identity();
  tilted_pose.block<3, 3>(0, 0) =
    Eigen::AngleAxisf(static_cast<float>(kPi / 6.0), Eigen::Vector3f::UnitX())
    .toRotationMatrix();

  const auto metrics =
    ll::computeAlignmentCorrectionMetrics(Eigen::Matrix4f::Identity(), tilted_pose);
  assert(std::abs(metrics.yaw_deg) < 1.0e-6);
  assert(std::abs(metrics.rotation_deg - 30.0) < 1.0e-4);
}
...
int main()
{
  ...
  test_rotation_metric_detects_tilt_hidden_from_yaw_metric();
  return 0;
}
