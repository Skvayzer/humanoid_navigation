  ...
  double voxel_leaf_size)
{
  ...
  if (downsample) {
    ...
    // Use PCL's pointer typedef: it is boost::shared_ptr in PCL 1.10 (Foxy)
    // and follows PCL's configured pointer type on newer distributions.
    pcl::PCLPointCloud2::Ptr raw_map_cloud_ptr(new pcl::PCLPointCloud2(raw_map_cloud));
    voxel_filter.setInputCloud(raw_map_cloud_ptr);
