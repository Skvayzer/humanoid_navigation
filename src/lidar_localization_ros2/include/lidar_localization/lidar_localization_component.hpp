...
public:
  ...
  builtin_interfaces::msg::Time normalizeInitialPoseStamp(
  ...
  std::mutex map_odom_transform_mutex_;
  geometry_msgs::msg::TransformStamped last_map_to_odom_transform_;
  bool have_last_map_to_odom_transform_{false};
  ...
  rclcpp_lifecycle::LifecyclePublisher<sensor_msgs::msg::PointCloud2>::SharedPtr
    initial_map_pub_;
  rclcpp_lifecycle::LifecyclePublisher<sensor_msgs::msg::PointCloud2>::SharedPtr
    alignment_seed_cloud_pub_;
  rclcpp_lifecycle::LifecyclePublisher<sensor_msgs::msg::PointCloud2>::SharedPtr
    alignment_proposed_cloud_pub_;
  rclcpp_lifecycle::LifecyclePublisher<geometry_msgs::msg::PoseStamped>::SharedPtr
    alignment_seed_pose_pub_;
  rclcpp_lifecycle::LifecyclePublisher<geometry_msgs::msg::PoseStamped>::SharedPtr
    alignment_proposed_pose_pub_;
  ...
  bool use_absolute_odom_pose_prediction_{false};
  double absolute_odom_correction_gain_{0.1};
  double absolute_odom_max_time_delta_sec_{0.05};
  std::size_t absolute_odom_history_size_{200};
  std::mutex absolute_odom_mutex_;
  std::deque<lidar_localization::TimedPoseMatrix> absolute_odom_history_;
  bool have_map_to_odom_correction_{false};
  Eigen::Matrix4f map_to_odom_correction_{Eigen::Matrix4f::Identity()};
  ...
  bool publish_alignment_debug_output_{false};
  ...
  void publishAlignmentDebugOutput(
    const builtin_interfaces::msg::Time & stamp,
    const pcl::PointCloud<pcl::PointXYZI>::Ptr & source_cloud,
    const lidar_localization::AlignmentAttempt & selected_attempt);
  Eigen::Matrix4f currentPoseMatrix() const;
  void cacheAbsoluteOdomPose(double stamp_sec, const Eigen::Matrix4f & odom_to_base);
  bool lookupAbsoluteOdomPose(double stamp_sec, Eigen::Matrix4f * odom_to_base);
  bool resetMapToOdomCorrection(
    const Eigen::Matrix4f & map_to_base,
    double stamp_sec);
  bool predictMapPoseFromAbsoluteOdom(
    double stamp_sec,
    Eigen::Matrix4f * map_to_base);
  bool filterAcceptedMapPoseWithAbsoluteOdom(
  ...
  void publishAlignmentStatus(
    ...
    double correction_rotation_deg = std::numeric_limits<double>::quiet_NaN(),
