...
class ReinitializationSupervisorNode(Node):
    ...
    def _publish_reset(self, candidate_index: int = 0) -> None:
        ...
        # Registration-refined candidates carry the full SE(3) attitude. Older
        # BBS-only replies remain valid and fall back to the last localizer pose.
        candidate_z = self._parse_finite_float(top.get("z"))
        candidate_roll_deg = self._parse_finite_float(top.get("roll_deg"))
        candidate_pitch_deg = self._parse_finite_float(top.get("pitch_deg"))
        if self.prefer_reset_default_z:
            z = self.reset_default_z
        elif candidate_z is not None:
            z = candidate_z
        else:
            z = self._last_pose_z if self._last_pose_z is not None else self.reset_default_z
        roll = (
            math.radians(candidate_roll_deg)
            if candidate_roll_deg is not None else self._last_pose_roll)
        pitch = (
            math.radians(candidate_pitch_deg)
            if candidate_pitch_deg is not None else self._last_pose_pitch)
