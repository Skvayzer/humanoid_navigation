...
struct MeasurementGateParams
{
  ...
  bool enable_correction_limit{false};
  double max_correction_translation_m{1.0};
  double max_correction_yaw_deg{10.0};
  double max_correction_rotation_deg{180.0};
  ...
  const MeasurementGateParamConfig & config)
{
  ...
  params.enable_correction_limit = config.enable_correction_limit;
  params.max_correction_translation_m = config.max_correction_translation_m;
  params.max_correction_yaw_deg = config.max_correction_yaw_deg;
  params.max_correction_rotation_deg = config.max_correction_rotation_deg;
...
inline MeasurementGateInput makeMeasurementGateInput(
  ...
  double correction_rotation_deg = 0.0)
{
  MeasurementGateInput input;
  input.fitness_score = fitness_score;
  input.accepted_gap_sec = accepted_gap_sec;
  input.seed_translation_since_accept_m = seed_translation_since_accept_m;
  input.correction_translation_m = correction_translation_m;
  input.correction_yaw_deg = correction_yaw_deg;
  input.consecutive_rejected_updates = consecutive_rejected_updates;
  input.correction_rotation_deg = correction_rotation_deg;
  return input;
}
  ...
  const MeasurementGateInput & input)
{
  ...
  if (params.enable_correction_limit) {
    const bool translation_exceeded =
      input.correction_translation_m > params.max_correction_translation_m;
    const bool yaw_exceeded = input.correction_yaw_deg > params.max_correction_yaw_deg;
    const bool rotation_exceeded =
      input.correction_rotation_deg > params.max_correction_rotation_deg;
    if (rotation_exceeded) {
      gate.status_level = kMeasurementGateWarn;
      gate.reject_measurement = true;
      gate.status_message = "correction_rotation_limit_rejected";
      return gate;
    }
    if (translation_exceeded || yaw_exceeded) {
      gate.status_level = kMeasurementGateWarn;
      gate.reject_measurement = true;
      if (translation_exceeded && yaw_exceeded) {
        gate.status_message = "correction_translation_and_yaw_limit_rejected";
      } else if (translation_exceeded) {
        gate.status_message = "correction_translation_limit_rejected";
      } else {
        gate.status_message = "correction_yaw_limit_rejected";
      }
      return gate;
    }
  }
