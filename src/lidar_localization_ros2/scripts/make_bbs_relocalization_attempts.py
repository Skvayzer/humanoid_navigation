...
def _read_netpbm_token(stream) -> bytes:
    token = bytearray()
    while True:
        byte = stream.read(1)
        if not byte:
        ...
        token.extend(byte)
    if not token:
        raise ValueError("unexpected end of Netpbm header/data")
    return bytes(token)


def _load_netpbm_grayscale(image_path: Path) -> np.ndarray:
    """Load P2/P5 PGM without relying on Pillow's build-specific codecs."""
    with image_path.open("rb") as stream:
        magic = _read_netpbm_token(stream)
        if magic not in (b"P2", b"P5"):
            raise ValueError(f"unsupported Netpbm format {magic!r}: {image_path}")
        width = int(_read_netpbm_token(stream))
        height = int(_read_netpbm_token(stream))
        max_value = int(_read_netpbm_token(stream))
        if width <= 0 or height <= 0 or not 0 < max_value <= 65535:
            raise ValueError(f"invalid PGM dimensions/range: {image_path}")

        count = width * height
        if magic == b"P2":
            values = np.fromiter(
                (int(_read_netpbm_token(stream)) for _ in range(count)),
                dtype=np.uint32,
                count=count,
            )
        else:
            bytes_per_sample = 1 if max_value < 256 else 2
            payload = stream.read(count * bytes_per_sample)
            if len(payload) != count * bytes_per_sample:
                raise ValueError(f"truncated PGM payload: {image_path}")
            dtype = np.uint8 if bytes_per_sample == 1 else np.dtype(">u2")
            values = np.frombuffer(payload, dtype=dtype, count=count)

    pixels = values.reshape((height, width)).astype(np.float32)
    if max_value != 255:
...
def load_occupancy_map(yaml_path: Path) -> OccupancyMap:
    ...
    pixels = _load_grayscale_image(image_path)
