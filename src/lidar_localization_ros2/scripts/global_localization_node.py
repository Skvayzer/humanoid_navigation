...
class GlobalLocalizationNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("matching_resolution_m", 0.0)
        self.declare_parameter("max_candidates", 16)
        self.declare_parameter("min_bbs_score", 0.0)
        ...
        self.declare_parameter("enable_final_registration_refinement", False)
        self.declare_parameter("final_refinement_candidate_count", 4)
        self.declare_parameter("final_ndt_scan_voxel_leaf_size", 0.25)
        self.declare_parameter("final_ndt_target_voxel_leaf_size", 0.25)
        ...
        config = GlobalLocalizationConfig(
            ...
            matching_resolution_m=float(
                self.get_parameter("matching_resolution_m").value),
            max_candidates=int(self.get_parameter("max_candidates").value),
            min_bbs_score=float(self.get_parameter("min_bbs_score").value),
            ...
            enable_final_registration_refinement=bool(
                self.get_parameter("enable_final_registration_refinement").value),
            final_refinement_candidate_count=int(
                self.get_parameter("final_refinement_candidate_count").value),
            final_ndt_scan_voxel_leaf_size=float(
                self.get_parameter("final_ndt_scan_voxel_leaf_size").value),
            final_ndt_target_voxel_leaf_size=float(
                self.get_parameter("final_ndt_target_voxel_leaf_size").value),
        )
        ...
        if enable_registration_scoring:
            if self.engine.registration_scorer is None:
                ...
                if self.engine.final_registration_scorer is not None:
                    self.get_logger().info(
                        "final registration refinement enabled: candidates=%d "
                        "scan_leaf=%.2f target_leaf=%.2f"
                        % (
                            config.final_refinement_candidate_count,
    ...
    def handle_query(self, request, response):
        ...
        scan_age_at_start_sec = glq.scan_age_sec(
            query_start_ros_sec, scan_stamp_sec)
        ...
        scan_age_at_end_sec = glq.scan_age_sec(query_end_ros_sec, scan_stamp_sec)
        candidate_age_sec = (
        ...
        for candidate in result.candidates:
            ...
            qx, qy, qz, qw = _quat_from_rpy(
                candidate.roll_rad, candidate.pitch_rad, candidate.yaw_rad)
        ...
        for candidate in result.candidates:
            entry = {
                ...
                "roll_deg": round(math.degrees(candidate.roll_rad), 2),
                "pitch_deg": round(math.degrees(candidate.pitch_rad), 2),
