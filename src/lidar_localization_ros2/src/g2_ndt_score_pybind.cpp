...
public:
  ...
  ScoreResultPy refine_candidate(
    py::array_t<double, py::array::c_style | py::array::forcecast> scan_xyz,
    double x,
    double y,
    double z,
    ...
    double yaw) const
  {
    ...
    {
      py::gil_scoped_release release;
      result = scorer_.score_xyz(ptr, count, x, y, z, roll, pitch, yaw);
    }
    return ScoreResultPy{
      result.fitness,
      result.converged,
...
PYBIND11_MODULE(g2_ndt_score, m)
{
  ...
  py::class_<ScoreResultPy>(m, "ScoreResult")
    ...
    .def_readonly("refined_roll", &ScoreResultPy::refined_roll)
    .def_readonly("refined_pitch", &ScoreResultPy::refined_pitch)
  ...
  py::class_<MapNdtScorerPy>(m, "MapNdtScorer")
    ...
    .def("refine_candidate", &MapNdtScorerPy::refine_candidate)
